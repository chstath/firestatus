...
var firestatus = {
	...
	onLoad: function(){
		...
		this.myspaceClient = myspaceClient;
	},
	...
	sendStatusUpdateFacebook: function(statusText, url) {
		...
		if (url)
			statusText += " " + url;
//		statusText = encodeURIComponent(statusText); //Somehow the status update fails if the status is encoded
		firestatus.facebookClient.updateStatus(statusText);
	},

	sendStatusUpdateMyspace: function(statusText, url) {
		firestatus.cons.logStringMessage("Starting myspace update...")
		if (url)
			statusText += " " + url;
//		statusText = encodeURIComponent(statusText); //Somehow the status update fails if the status is encoded
		firestatus.myspaceClient.updateStatus(statusText);
	},
