...
//		facebookClient.getNewSessionAndUpdate(status);
	    ...
	    var req = new XMLHttpRequest();
		req.open("PUT", "http://api.msappspace.com/v1/users/chstath/status.json", true);
		req.onreadystatechange = function () {
			if (req.readyState == 4) {
			     switch(req.status) {
				 	case 200:
				 		...
				 		break;
					case 400:
						firestatus.cons.logStringMessage("Myspace response: Bad Request");
						break;
					case 401:
						firestatus.cons.logStringMessage("Myspace response: Not Authorized");
						break;
					case 403:
						firestatus.cons.logStringMessage("Myspace response: Forbidden");
						break;
					case 404:
						firestatus.cons.logStringMessage("Myspace response: Not Found");
						break;
					case 500:
						firestatus.cons.logStringMessage("Myspace response: Internal Server Error");
						break;
					case 502:
						firestatus.cons.logStringMessage("Myspace response: Bad Gateway");
						break;
					case 503:
						firestatus.cons.logStringMessage("Myspace response: Service Unavailable");
						break;
					default:
						firestatus.cons.logStringMessage("Unknown Myspace status: " + req.status);
						firestatus.cons.logStringMessage("Myspace response: " + req.responseText);
				 }
			}
		};
